 ...
 */
class ProjectAllTimeSheet extends CActiveRecord
{
    /**
    ...
    public function search()
    {
        ...
        $criteria->alias = 'p';
        $criteria->select = 'CONCAT(cast(time_hrs as decimal(10,2))  + cast( (time_mins/60) as decimal(10,2)) ," hrs") as totalhrs,timesheet_date,p.id,p.project_id,p.user_id,espl_employee_details.name,p.time_hrs,p.time_mins,p.timesheet_date,p.created_date,p.modified_date';
        ...
        $criteria->join= 'inner join espl_employee_details on espl_employee_details.user_id = p.user_id';
        $criteria->addCondition('p.project_id = ' . $projectid);
