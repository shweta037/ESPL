...
class ProductAssignController extends Controller
{
	...
	{
		$model=new ProductAssign;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['ProductAssign']))
		{
			$model->attributes=$_POST['ProductAssign'];
            $date = date('Y-m-d H:i:s');
            $model['created_date'] = $date;
            $model['created_by'] = Yii::app()->user->name;

            if($model->save()) {
                $url = Yii::app()->createUrl('productAssign/admin');
	...
	{
		$dataProvider=new CActiveDataProvider('ProductAssign');
        $this->render('/include/dashboard_header');
        $this->render('/include/dashboard_leftbar');
	...
	{
		$model=new ProductAssign('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['ProductAssign']))
			$model->attributes=$_GET['ProductAssign'];
	 ...
	 * @return ProductAssign the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=ProductAssign::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param ProductAssign $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='product-assign-form')
		{
