...
class Holidays extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
		    array('holiday_name ,holiday_date','required'),
			//array('is_deleted', 'numerical', 'integerOnly'=>true),
			//array('holiday_name', 'length', 'max'=>512),
			array('holiday_date, created_date, modified_date', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, holiday_name, holiday_date, is_deleted, created_date, modified_date', 'safe', 'on'=>'search'),
		);
	}
